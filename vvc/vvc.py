...
def init_keras():
    ...
    if keras_session is None:
        # set the modified tf session as backend in keras
        keras_session = get_session()
        keras.backend.tensorflow_backend.set_session(keras_session)


def format_img_yolo(img, img_min_side):
...
class VVC(object):
    ...
    def count(self, video_name, frame_rate_factor=1, filter_tags=None, show_obj_id=True):
        ...
        input_video_file = Path(vvc_config.video_folder).joinpath(video_name)

        video_name_no_suffix = Path(input_video_file).stem

        output_folder = Path(vvc_config.output_folder).joinpath(video_name_no_suffix).joinpath(self.tracker.name)
        output_folder.mkdir(exist_ok=True)
        output_video_file = output_folder.joinpath(self.model_name + '.mp4')
        ...
        with tqdm(total=total_frames, unit='frame') as pbar:
            ...
            for frame in reader:
                ...
                # Resize input image
                x, scale_factor = format_img_yolo(img, 600)
        ...
        json_file = output_video_file.with_suffix('.json')
