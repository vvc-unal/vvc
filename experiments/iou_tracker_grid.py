...
if __name__ == '__main__':
    ...
    for iou_threshold in np.arange(0.3, 0.8, 0.1):
        for dectection_threshold in np.arange(0.5, 0.9, 0.1):
            accs = []
            v_names = []
            logging.info('Tracker params: iou_threshold={}, dectection_threshold={}'.format(iou_threshold,
                                                                                            dectection_threshold))
            tracker = IOUTracker(iou_threshold=iou_threshold, dectection_threshold=dectection_threshold)
            ...
            results = results.append(summary.rename(columns=motmetrics.io.motchallenge_metric_names), ignore_index=True)

    results.to_csv(Path(config.output_folder).joinpath('iou_tracker_grid.csv'), index=False, float_format='%.2f')
